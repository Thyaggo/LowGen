...
class EncodecModel(nn.Module):
    ...
    def encode(self, x: torch.Tensor, emb_only: bool = False) -> tp.List[EncodedFrame]:
        ...
        for offset in range(0, length, stride):
            ...
            if emb_only:
                encoded_frames.append(self._encode_emb(frame))
            else:
    ...
    def _encode_emb(self, x: torch.Tensor) -> EncodedFrame:
        length = x.shape[-1]
        duration = length / self.sample_rate
        assert self.segment is None or duration <= 1e-5 + self.segment
